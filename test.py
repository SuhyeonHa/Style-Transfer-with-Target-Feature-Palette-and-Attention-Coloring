...
def style_transfer(net, content, style, args, alpha=1.0):
...
def get_args_parser():
    ...
    parser.add_argument('--test_mode', type=str, default='single_style_transfer',
                        choices=['single_style_transfer', 'multi_style_transfer'])
    ...
    if args.test_mode == 'single_style_transfer':
        for content_path in content_paths:
            for style_path in style_paths:
                content = trans(Image.open(str(content_path))).to(device).unsqueeze(0)
                style = trans(Image.open(str(style_path))).to(device).unsqueeze(0)
        ...
        content = trans(Image.open(str(content_paths[0]))).to(device).unsqueeze(0)
